<?php
// Vérifie si l'utilisateur est authentifié au niveau du site
session_start();
...
if(isset($_GET['id']) AND !empty($_GET['id'])){
  ...
  $checkIfQuestionExists = $bdd->prepare('SELECT id_questions, id_auteur FROM questions WHERE id_questions = ?');
  ...
  if($checkIfQuestionExists->rowCount() > 0) {
    ...
    if($questionsInfos['id_auteur'] == $_SESSION['id']){
      ...
      $deleteThisQuestion = $bdd->prepare('DELETE FROM questions WHERE id_questions = ?');
